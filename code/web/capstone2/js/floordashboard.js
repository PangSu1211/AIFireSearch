const FLOOR_DATA = {
  1: {
    cameras: [
      { id: '1F-01', x: 4, y: 6, people: 1, trust: 93, place: '입구' },
      { id: '1F-02', x: 26, y: 6, people: 2, trust: 90, place: '회의실 앞' },
      { id: '1F-03', x: 36, y: 6, people: 3, trust: 87, place: '복도' },
      { id: '1F-04', x: 48, y: 6, people: 2, trust: 64, place: '사무실 A' },
      { id: '1F-05', x: 66, y: 6, people: 3, trust: 90, place: '우측 복도' },
      { id: '1F-06', x: 4, y: 20, people: 0, trust: 92, place: '좌측 복도' },
      { id: '1F-07', x: 28, y: 48, people: 4, trust: 57, place: '사무실 B' },
      { id: '1F-08', x: 46, y: 48, people: 2, trust: 26, place: '중앙 복도' },
      { id: '1F-09', x: 60, y: 48, people: 3, trust: 52, place: '우측 사무실' },
      { id: '1F-10', x: 78, y: 48, people: 0, trust: 82, place: '끝 복도' },
    ],
  },
  2: {
    cameras: [
      { id: '2F-01', x: 6, y: 8, people: 0, trust: 88, place: '입구' },
      { id: '2F-02', x: 24, y: 10, people: 1, trust: 76, place: '복도' },
      { id: '2F-03', x: 47, y: 10, people: 3, trust: 91, place: '사무실' },
      { id: '2F-04', x: 64, y: 34, people: 2, trust: 58, place: '회의실' },
      { id: '2F-05', x: 80, y: 52, people: 1, trust: 63, place: '출구' },
    ],
  },
  3: {
    cameras: [
      { id: '3F-01', x: 10, y: 18, people: 1, trust: 95, place: '좌측' },
      { id: '3F-02', x: 40, y: 22, people: 2, trust: 81, place: '중앙' },
      { id: '3F-03', x: 70, y: 42, people: 0, trust: 58, place: '우측' },
    ],
  },
};
...
function getFloorTotalPeople(floor) {
  return floor.cameras.reduce((sum, camera) => sum + camera.people, 0);
}

function getAlertCameras(floor) {
  return floor.cameras.filter(camera => getStatus(camera.trust) !== 'green');
}
...
function renderCameraLayer() {
  ...
  floor.cameras.forEach(camera => {
    ...
    box.type = 'button';
    box.className = `camera-box status-${status}`;
...
function updateSelectedPanel() {
  ...
  const status = getStatus(selectedCamera.trust);

  document.getElementById('selectedId').textContent = selectedCamera.id;
  document.getElementById('selectedMeta').textContent =
    `${currentFloor}층 ${selectedCamera.place} · 인원 ${selectedCamera.people}명 · 신뢰도 ${selectedCamera.trust}%`;

  const statusEl = document.getElementById('selectedStatus');
  statusEl.textContent = getStatusText(status);
  statusEl.className = `selected-status ${status}`;
}

function updateSummary() {
  const floor = FLOOR_DATA[currentFloor];
  const alertCameras = getAlertCameras(floor);
  const alertCount = alertCameras.length;
  const totalPeople = getFloorTotalPeople(floor);
  ...
  if (chip) {
    ...
    if (alertCount === 0) {
      ...
      chip.textContent = `주의 필요 ${alertCount}`;
...
function renderAll() {
  ...
  selectedCamera =
    floor.cameras.find(camera => camera.id === initialCameraId) ||
    floor.cameras[0];
